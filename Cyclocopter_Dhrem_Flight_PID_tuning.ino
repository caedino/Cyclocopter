...
float GyroErrorX = -5.08;
float GyroErrorY = -1.97;
float GyroErrorZ = 2.95;
...
float Kp_roll_angle = 0.07;    //Roll P-gain - angle mode 
float Ki_roll_angle = 0.1;    //Roll I-gain - angle mode
float Kd_roll_angle = 0.025;   //Roll D-gain - angle mode (has no effect on controlANGLE2)
float B_loop_roll = 0.9;      //Roll damping term for controlANGLE2(), lower is more damping (must be between 0 to 1)
float Kp_pitch_angle = 0.07;   //Pitch P-gain - angle mode
...
float Kp_roll_rate = 0.05;    //Roll P-gain - rate mode
float Ki_roll_rate = 0.1;     //Roll I-gain - rate mode
float Kd_roll_rate = 0.0002;  //Roll D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!)
float Kp_pitch_rate = 0.05;   //Pitch P-gain - rate mode
float Ki_pitch_rate = 0.1;    //Pitch I-gain - rate mode
...
void controlMixer() {
  ...
  m3_command_scaled = thro_des + pitch_PID - roll_PID; //Back Right
  m4_command_scaled = thro_des + pitch_PID + roll_PID; //Back Left
  m5_command_scaled = 0;
  m6_command_scaled = 0;

  //0.5 is centered servo, 0.0 is zero throttle if connecting to ESC for conventional PWM, 1.0 is max throttle
  s1_command_scaled =  yaw_PID + 0.5;
  s2_command_scaled =  yaw_PID + 0.5;
  s3_command_scaled =  yaw_PID + 0.5;
  s4_command_scaled =  yaw_PID + 0.5;
  s5_command_scaled = 0;
  s6_command_scaled = 0;
  s7_command_scaled = 0;
...
void scaleCommands() {
  ...
  m1_command_PWM = constrain(m1_command_PWM, 125, 200);
  m2_command_PWM = constrain(m2_command_PWM, 125, 200);
  m3_command_PWM = constrain(m3_command_PWM, 125, 200);
  m4_command_PWM = constrain(m4_command_PWM, 125, 200);
  m5_command_PWM = constrain(m5_command_PWM, 125, 200);
  m6_command_PWM = constrain(m6_command_PWM, 125, 200);
